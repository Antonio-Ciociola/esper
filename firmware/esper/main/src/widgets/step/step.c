 ...
 *      DEFINES
 *********************/
#define MY_CLASS (&step_class)

/**********************
 *      TYPEDEFS
 **********************/

typedef struct _step_t step_t;

/**********************
 *  STATIC PROTOTYPES
 **********************/
static void step_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj);
static void step_event(const lv_obj_class_t *class_p, lv_event_t *e);

/**********************
 *  STATIC VARIABLES
 **********************/

const lv_obj_class_t step_class = {
    .name = "step",
    .base_class = &lv_obj_class,
    .constructor_cb = step_constructor,
    .event_cb = step_event,
    .instance_size = sizeof(step_t),
    .width_def = 20,
    .height_def = 12,
...
static void step_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
    ...
    // led->bright = step_BRIGHT_MAX;

    step->num_notes = 12 * 5 + 1;
}
...
static void step_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
    ...
    else if (code == LV_EVENT_DRAW_POST)
    {
        ...
        if (step->current)
        {
            ...
            line_dsc.color = COLOR_FG; // Adjust color if needed
            line_dsc.width = 2;
            ...
            line_dsc.p1 = points[2];
            line_dsc.p2 = points[0];
            lv_draw_line(layer, &line_dsc);
        }
    }
    ...
    else if (code == LV_EVENT_KEY)
    {
        ...
        if (c == LV_KEY_RIGHT || c == LV_KEY_DOWN)
            ...
            dir = -1;
        int new_sel = step->sel + dir;
        new_sel = new_sel < 0 ? 0 : new_sel;
        new_sel = new_sel > step->num_notes-1 ? step->num_notes-1 : new_sel;
        step->sel = new_sel;
        lv_obj_invalidate(obj);
    }
}
